...
# Copyright: (c) 2020, Joao Vale <jpvale@gmail.com>
...
def merge_dict(destination, source):
    for key, value in source.items():
        ...
        # items have different types, override existing
        if type(destination[key]) is not type(value):
            destination[key] = value
            continue

        # item is a dict, merge recursively
...
class ActionModule(ActionBase):
    ...
    VALID_FILE_ARGUMENTS = ['files', '_raw_params']
    VALID_ALL = ['name']
    ...
    def _set_args(self):
        ...
        self.source_files = self._task.args.get('files', [])
        if not self.source_dir and not self.source_files:
            self.source_files = [self._task.args.get('_raw_params')]
        ...
        if self.source_dir:
            ...
            if not path.exists(self.source_dir):
                ...
                for root_dir, filenames in self._traverse_dir_depth():
                    failed, err_msg, new_data = (self._load_files_in_dir(root_dir, filenames))
                    if failed:
                        break
                    merge_dict(results, new_data)
        else:
            try:
                for filename in self.source_files:
                    filename = self._find_needle('vars', filename)
                    failed, err_msg, new_data = (self._load_files(filename))
                    if not failed:
                        merge_dict(results, new_data)
